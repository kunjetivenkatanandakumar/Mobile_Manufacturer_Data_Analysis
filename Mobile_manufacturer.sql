...
WHERE 
    L.Country = 'US'
    AND (M.Manufacturer_Name LIKE 'Samsung%' OR M.Manufacturer_Name LIKE '%SAMSUNG%')
GROUP BY 
    L.[State]
ORDER BY 
    Total_Quantity DESC;
...
SELECT
    M.Manufacturer_Name,
    MO.Model_Name,
    AVG(MO.Unit_price) AS Average_Price
FROM FACT_TRANSACTIONS F
INNER JOIN DIM_MODEL MO ON F.IDModel = MO.IDModel
INNER JOIN DIM_MANUFACTURER M ON MO.IDManufacturer = M.IDManufacturer
WHERE M.IDManufacturer IN (SELECT IDManufacturer FROM TopManufacturers)
...
SELECT 
    C.Customer_Name,
    AVG(F.TotalPrice) AS Average_Amount_Spent
FROM FACT_TRANSACTIONS F
...
GROUP BY 
    C.Customer_Name
HAVING 
    AVG(F.TotalPrice) > 500;

--Q6--END

--Q7--BEGIN

WITH TopModelsPerYear AS (
    SELECT 
        YEAR(F.Date) AS SaleYear,
        F.IDModel,
        SUM(F.Quantity) AS TotalQuantity,
        RANK() OVER (PARTITION BY YEAR(F.Date) ORDER BY SUM(F.Quantity) DESC) AS RankByQuantity
    FROM FACT_TRANSACTIONS F
...
WHERE MO.IDModel IN (
    SELECT IDModel
    FROM TopModelsPerYear
    WHERE RankByQuantity <= 5
    GROUP BY IDModel
    HAVING COUNT(DISTINCT SaleYear) = 3 -- Means model appeared in top 5 all three years
);
...
WITH ManufacturerSales AS (
    ...
    GROUP BY YEAR(F.Date), M.IDManufacturer, M.Manufacturer_Name
)
SELECT 
    SaleYear,
    Manufacturer_Name,
...
Sales2010 AS (
    SELECT DISTINCT M.IDManufacturer, M.Manufacturer_Name
    FROM FACT_TRANSACTIONS F
    INNER JOIN DIM_MODEL MO ON F.IDModel = MO.IDModel
    INNER JOIN DIM_MANUFACTURER M ON MO.IDManufacturer = M.IDManufacturer
...
SELECT
    S10.Manufacturer_Name
FROM Sales2010 S10
LEFT JOIN Sales2009 S09 ON S10.IDManufacturer = S09.IDManufacturer
WHERE S09.IDManufacturer IS NULL;
...
TopCustomers AS (
    SELECT TOP 100
        IDCustomer,
        SUM(Total_Spend) AS TotalSpendAllYears
    FROM CustomerYearlyStats
    GROUP BY IDCustomer
    ORDER BY TotalSpendAllYears DESC
),
CustomerSpendingWithChange AS (
    SELECT
        C.IDCustomer,
        C.SaleYear,
        C.Avg_Spend,
        C.Avg_Quantity,
        C.Total_Spend,
        LAG(C.Total_Spend) OVER (PARTITION BY C.IDCustomer ORDER BY C.SaleYear) AS Prev_Year_Spend,
        TC.TotalSpendAllYears
    FROM CustomerYearlyStats C
    INNER JOIN TopCustomers TC ON C.IDCustomer = TC.IDCustomer
)
SELECT 
    IDCustomer,
    SaleYear,
    ...
    CASE 
        ...
        ELSE ((Total_Spend - Prev_Year_Spend) * 100.0) / Prev_Year_Spend
    END AS Percentage_Change_Spend
FROM CustomerSpendingWithChange
ORDER BY IDCustomer, SaleYear;
